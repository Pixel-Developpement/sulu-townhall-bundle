...
class FlashInfoController extends AbstractController
{
    ...
    public function flashInfo(EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        if(empty($flashInfos)){
            return new JsonResponse(["success" => true, "template" => $this->renderView("flash_info/empty.html.twig")]);
        }
        $activeFlashInfos = [];
        ...
        return new JsonResponse(["success" => true, "template" => $this->renderView("flash_info/index.html.twig", ["activeFlashInfos" => $activeFlashInfos])]);
    }
}
